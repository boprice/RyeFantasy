<script>
    ...
    import StatsAllTimeRecords from './StatsAllTimeRecords.svelte';
    import StatsPerSeasonRecords from './StatsPerSeasonRecords.svelte';
...
<div class="rankingsWrapper">
    ...
    {#if display == "allTime"}
        {#if leagueWeekHighs?.length}
            <StatsAllTimeRecords transactionTotals={totals} {allTimeClosestMatchups} {allTimeBiggestBlowouts} {leagueManagerRecords} {leagueWeekHighs} {leagueWeekLows} {leagueTeamManagers} {mostSeasonLongPoints} {leastSeasonLongPoints} {key} />
        {:else}
            <p class="empty">No stats <i>yet</i>...</p>
        {/if}
    {:else}
        <StatsPerSeasonRecords transactionTotals={totals} {leagueRosterRecords} {seasonWeekRecords} {leagueTeamManagers} {currentYear} {lastYear} {key} />
    {/if}
</div>
